...
class DailyPolicy(BasePolicy):
    ...
    def rise_factor(self, item):
        ...
        if item['vary_portion'] >= 1.00 and item['close_price'] > item['open_price']:
            sid_str = str(sid)
            if sid_str not in self.datamap['past_data']:
                print "op=not_exists_pastdata sid=" + str(sid)
                return
            ...
            print format_log("daily_rise_factor", stock_rise_map)
    ...
    def day_trend(self, item):
        ...
        print format_log("daily_day_trend", trend_info)
