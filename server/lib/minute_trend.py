...
class MinuteTrend(object):
	...
	def core(self, daily_item, minute_items):
		...
		if trend_length >= 3:
			...
			if (MinuteTrend.TREND_RISE == latest_trend_item['trend']) or (MinuteTrend.TREND_WAVE == latest_trend_item['trend'] and 1 == latest_trend_item['direction']):
			...
			elif (MinuteTrend.TREND_FALL == latest_trend_item['trend']) or (MinuteTrend.TREND_WAVE == latest_trend_item['trend'] and -1 == latest_trend_item['direction']):
		...
		# TODO: 输出日志			
		trend_stage = {"sid": self.sid, "code": daily_item['code'], "daily_trend": daily_trend, "time": minute_items[latest_trend_item['end']]['time'], "op": op, "price_range": price_range, "trend_item": latest_trend_item, "daily_item": daily_item} 
	...
	def split_list(price_list):
		...
		while end_index < len(delta_list):
			if delta_list[end_index] * direction < 0:
				range_list.append((start_index, end_index-1, direction, end_index-start_index, max(price_list[start_index :end_index]), min(price_list[start_index:end_index])))
				start_index = end_index-1
				direction = -1 * direction
			end_index += 1
		
		# 最后一段	
		range_tuple = (start_index, len(delta_list) - 1, direction, len(delta_list) - start_index, max(price_list[start_index :len(delta_list)]), min(price_list[start_index:len(delta_list)]))
		...
		while loop:
			...
			while i < len(range_list):
				...
				# item与左右两边的趋势相反, 3段合并到一起
					...
					'''
					if (last_item[3] >= 3 and next_item[3] >= 3) or (last_item[2] == 1 and next_item[4] >= last_item[4]) or (last_item[2] == -1 and next_item[5] <= last_item[5]):
						merge_item = (last_item[0], next_item[1], last_item[2], next_item[1] - last_item[0] + 1, max(last_item[4], next_item[4]), min(last_item[5], next_item[5]))
						...
						if loop_count <= 2:
							...
							if (item[2] == -1 and next_item[4] <= item[4]) or (item[2] == 1 and next_item[5] >= item[5]):
								merge_item = (item[0], next_item[1], item[2], next_item[1] - item[0] + 1, max(item[4], next_item[4]), min(item[5], next_item[5])) 
					...
					next_item = range_list[i+1]
					if next_item[3] >= 3:
						merge_item = (item[0], next_item[1], next_item[2], next_item[1] - item[0] + 1, max(item[4], next_item[4]), min(item[5], next_item[5]))
						result_list.append(merge_item)
						i += 2
					else:
						i += 1
	...
	def parse(self, daily_item, minute_items):
		...
		for item in combined_list:
			...
			if need_merge and last_item and trend == last_item['trend']:
				...
				merge_item['vary_portion'] = (price_list[merge_item['end']] - price_list[merge_item['start']]) / price_list[merge_item['start']] * 100
...
if __name__ == "__main__":
	...
	def get_datamap(code, index):
		...
		daily_item = {"sid": 212, "code": "WUBA", "day": 20160311, "open_price": 56.36, "last_close_price": last_close_price, "close_price": now_item['price'], "time": now_item['time'], "high_price": high_price, "low_price": low_value, "vary_price": now_item['price'] - last_close_price, "vary_portion": (now_item['price'] - last_close_price)/last_close_price*100}
		#print daily_item
